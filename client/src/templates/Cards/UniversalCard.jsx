// src/templates/Cards/UniversalCard.jsx

import React, { useState, useRef } from "react";
import {
  FaPlus, FaTrash, FaRegFileAlt, FaImage,
  FaChartBar, FaMoneyBillWave, FaChevronLeft, FaChevronRight, FaDownload
} from "react-icons/fa";

/**
 * UniversalCard - A reusable, feature-rich project/task card.
 * @param {string} title - Title for the card (e.g., "Paloma Launcher")
 * @param {JSX.Element} icon - Icon element (optional)
 */
const UniversalCard = ({ title = "Universal Card", icon }) => {
  // Checklist state
  // Each item: { label, done, subitems: [{label, done}] }
  const [checklist, setChecklist] = useState([]);
  const [newChecklistItem, setNewChecklistItem] = useState("");
  const [addingSubToIndex, setAddingSubToIndex] = useState(null);
  ...
  // Calculate weighted progress: subitems factor into each master item's progress
  const calculateProgress = () => {
    if (checklist.length === 0) return 0;

    const progresses = checklist.map(item => {
      if (item.subitems && item.subitems.length > 0) {
        const doneSubs = item.subitems.filter(sub => sub.done).length;
        return doneSubs / item.subitems.length;
      }
      return item.done ? 1 : 0;
    });

    const avgProgress = progresses.reduce((sum, val) => sum + val, 0) / progresses.length;
  ...
  const handleAddChecklistItem = () => {
    if (newChecklistItem.trim()) {
      setChecklist([
        ...checklist,
        { label: newChecklistItem.trim(), done: false, subitems: [] }
      ]);
      setNewChecklistItem("");
    }
  };
  ...
  const handleDeleteChecklistItem = (idx) => {
    ...
    if (addingSubToIndex === idx) {
      setAddingSubToIndex(null);
      setNewSubItemText("");
    }
  };

  // Toggle master item and update all sub-items accordingly
  const handleToggleChecklistItem = (idx) => {
    setChecklist(list =>
      list.map((item, i) => {
        if (i === idx) {
          const newDone = !item.done;
          const newSubs = item.subitems?.map(sub => ({ ...sub, done: newDone })) || [];
          return { ...item, done: newDone, subitems: newSubs };
        }
        return item;
      })
    );
  ...
  // Sublist handlers
  const handleAddSubItemClick = (idx) => {
    if (addingSubToIndex === idx) {
      setAddingSubToIndex(null);
      setNewSubItemText("");
    } else {
      setAddingSubToIndex(idx);
      setNewSubItemText("");
    }
  };

  const handleAddSubItemChange = (e) => {
    setNewSubItemText(e.target.value);
  };

  const handleAddSubItemSubmit = (idx) => {
  if (newSubItemText.trim()) {
    setChecklist(list =>
      list.map((item, i) => {
        if (i === idx) {
          const newSubitems = item.subitems ? [...item.subitems] : [];
          newSubitems.push({ label: newSubItemText.trim(), done: false });
    ...
    setNewSubItemText("");
    // Keep input open by NOT calling setAddingSubToIndex(null) here
  }
};

  // Toggle sub-item and update master accordingly
  const handleSubItemToggle = (parentIdx, subIdx) => {
    setChecklist(list =>
      list.map((item, i) => {
        if (i === parentIdx) {
          const newSubs = item.subitems.map((subitem, si) =>
            si === subIdx ? { ...subitem, done: !subitem.done } : subitem
          );
          // Don't mark master done here since progress is calculated separately
  ...
  const handleSubItemDelete = (parentIdx, subIdx) => {
    setChecklist(list =>
      list.map((item, i) => {
        if (i === parentIdx) {
  ...
  return (
    <div
      className="w-full h-full bg-black shadow-2xl rounded-lg border-2 border-[#6a7257] flex flex-col"
    ...
    >
      ...
      >
        {/* Checklist */}
        <div className="flex flex-col items-start justify-start border-r border-[#35392e] px-6 py-6 bg-black overflow-auto">
            ...
            {checklist.map((item, idx) => (
              <li key={idx} className="flex flex-col gap-1">
                <div className="flex items-center gap-2">
                  ...
                  <span
                    className={`text-sm ${
                      item.done ? "line-through text-[#949c7f]" : "text-[#f3f4f1]"
                    }`}
                  >
                    {item.label}
                  </span>
                  ...
                  {/* Add sublist toggle button */}
                  <button
                    onClick={() => handleAddSubItemClick(idx)}
                    title="Add Sub-item"
                    className="ml-1 text-[#b0b79f] hover:text-green-400"
                    style={{ fontSize: 14 }}
                  >
                    <FaPlus />
                  </button>
                ...
                <ul className="ml-6 space-y-1">
                  {item.subitems && item.subitems.map((subitem, subIdx) => (
                    <li key={subIdx} className="flex items-center gap-2">
                      <input
                        ...
                        onChange={() => handleSubItemToggle(idx, subIdx)}
                        className="accent-[#6a7257] w-3 h-3"
                      />
                      <span
                        className={`text-xs ${
                          subitem.done ? "line-through text-[#949c7f]" : "text-[#f3f4f1]"
                        }`}
                      >
                        {subitem.label}
                      </span>
                      <button
                        onClick={() => handleSubItemDelete(idx, subIdx)}
                  ...
                  {/* Input to add new subitem */}
                  {addingSubToIndex === idx && (
                    <li className="flex items-center gap-2">
                      <input
                        type="text"
                        value={newSubItemText}
                        onChange={handleAddSubItemChange}
                        onKeyDown={(e) => {
                          if (e.key === "Enter") {
                            handleAddSubItemSubmit(idx);
                          }
                          if (e.key === "Escape") {
                            setAddingSubToIndex(null);
                            setNewSubItemText("");
                        ...
                        className="flex-1 rounded bg-[#23241f] border border-[#6a7257] text-xs text-[#faf5e6] px-2 py-1 focus:outline-none"
                        autoFocus
                      />
                      <button
                        onClick={() => handleAddSubItemSubmit(idx)}
                      ...
                      <button
                        ...
                        className="ml-1 px-2 py-1 rounded bg-[#22261e] border border-[#6a7257] text-[#cfddb3] hover:bg-[#b74747] hover:text-black flex items-center"
                        title="Cancel"
                        style={{ fontSize: 12 }}
                      >
                        Cancel
