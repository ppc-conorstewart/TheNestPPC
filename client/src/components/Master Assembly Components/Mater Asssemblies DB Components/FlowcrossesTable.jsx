...
// FLOWCROSSES list table (no column header; parent renders it)
// ============================================================
...
function statusPill(val) {
  ...
  if (/^ma\s*\(mi-\d+\)$/.test(val || '')) return { bg: '#2a1f3a', fg: missilePurple, label: val };
...
export default function FlowcrossesTable(props) {
  return (
    <div style={{ overflowY: 'auto', overflowX: 'hidden', flex: 1, minHeight: 0 }}>
      <TableBody {...props} type="Flowcrosses" zebraTextPalette={['#e3a35c', '#f0b675']} leftStripe={accentAmber} />
    </div>
  );
}
...
function TableBody({
  ...
  setHoverKey = () => {},
  onView = () => {},
  zebraTextPalette = ['#e3a35c', '#f0b675'],
  leftStripe = accentAmber,
}) {
  return (
    <table style={{ width: '100%', borderCollapse: 'separate', borderSpacing: 0, fontSize: '0.64rem', tableLayout: 'fixed' }}>
      ...
      <tbody>
        {names.map((name, idx) => {
          ...
          const zebraTextColor = zebraTextPalette[idx % 2];
          ...
          return (
            ...
            >
              <td style={{ ...assemblyCell, paddingLeft: 8, position: 'relative', color: zebraTextColor }}>
                <span style={{ position: 'absolute', left: 0, top: 0, bottom: 0, width: 3, background: `linear-gradient(180deg, ${leftStripe}, ${leftStripe}44)` }} />
              ...
              <td style={tdCell}><span style={chip(pill.bg, pill.fg)}>{pill.label}</span></td>
