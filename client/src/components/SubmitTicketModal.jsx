// src/components/SubmitTicketModal.jsx
...
export default function SubmitTicketModal({
  ...
  // Helper to format for <input type="date" />
  ...
  useEffect(() => {
    ...
    // When editing, always format date fields to YYYY-MM-DD for inputs
    setForm({
      ...initialData,
      neededBy: formatInputDate(initialData.neededBy),
      // add any other date fields you use here:
    });
    setError('');
  }, [open, initialData, setError]);
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-70 z-50 flex items-center justify-center">
      ...
      >
        ...
        >
          {editingId ? "EDIT SOURCING TICKET" : "SUBMIT SOURCING TICKET"}
        </h2>
        {error && (
        ...
        <form onSubmit={handleSubmit} className="space-y-3">
          ...
          {/* NEEDED BY */}
          <div>
            <label className="block text-white text-xs font-bold uppercase mb-1 text-left" style={{ fontSize: 13 }}>
              NEEDED BY:
            </label>
            <input
              type="date"
              name="neededBy"
              value={formatInputDate(form.neededBy)}
              onChange={handleField}
              className="w-full bg-[#1B1D16] text-center text-white border border-[#949c7f] px-3 py-2 rounded-lg text-sm uppercase tracking-wide"
              required
              style={{ fontWeight: 600, letterSpacing: 2, fontSize: 15 }}
            />
          </div>
