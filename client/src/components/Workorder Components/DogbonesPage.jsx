// src/components/DogbonesPage.jsx
...
export default function DogbonesPage({
  metadata,
  assets,
  buildQtys,
  setBuildQtys,
  ...
  const tabs = ['DB-1', 'DB-2'];

  // Void toggles for slots 1–10
  ...
  // Categories & bucketed assets
  const categories = ['Valves', 'Adapters', 'Weco', 'Spools', 'Other'];
  const bucketed = useMemo(() => {
    const buckets = { Valves: new Set(), Adapters: new Set(), Weco: new Set(), Spools: new Set(), Other: new Set() };
    assets.forEach(a => {
      const nm = a.name || '';
    ...
    return Object.fromEntries(
      Object.entries(buckets).map(([k, set]) => [k, Array.from(set).sort((a, b) => a.localeCompare(b))])
    );
  }, [assets]);

  // Summarize availability for a given slot key
  const getSummary = key => {
    const sel = metadata[key] || '';
    const summary = {};
    assets
      .filter(a => a.name === sel && a.status === 'Available')
      .forEach(a => { summary[a.location] = (summary[a.location] || 0) + 1 });
    const inUse = assets.filter(a => a.name === sel && /in[\s-]?use/i.test(a.status)).length;
  ...
  function Slot({ n }) {
    ...
    const chosenCategory = metadata[catKey] || '';
    ...
    if (!chosenCategory) {
      ...
      options = [
        ...
        ...bucketed[chosenCategory].map(name => ({ value: name, label: name }))
      ];
        ...
        <div className="flex items-center gap-1 uppercase">
          ...
          {!isVoid ? (
            <Select
              ...
              isClearable
              className="flex-1 text-[7px] bg-black"
              styles={{
                control: base => ({ ...base, backgroundColor: '#111', borderColor: '#374151', minHeight: '1.2rem', fontSize: '0.6rem' }),
                menu: base => ({ ...base, backgroundColor: '#111' }),
                option: (base, { isFocused }) => ({ ...base, backgroundColor: isFocused ? '#374151' : '#111', fontSize: '0.6rem' })
              }}
    ...
    <div className="flex h-full bg-black uppercase text-[9px]">
      ...
      <div className="flex-1 bg-[#111] border border-[#6a7257] p-3 flex flex-col overflow-auto">
          ...
          <div className="flex items-center space-x-2">
            ...
            <input
              ...
              onChange={e => {
                ...
                setBuildQtys(qs => qs.map((x, i) => (i === activeTab ? v : x)));
              }}
              className="w-16 px-2 py-1 rounded bg-black text-xl text-white border border-[#374151]"
            />
          </div>
            ...
            {tabs.map((lbl, i) => (
              <button
                ...
                className={`px-3 py-1 font-bold text-sm rounded ${
                  activeTab === i ? 'bg-[#6a7257] text-black' : 'bg-[#333] text-white'
                }`}
              >
                {lbl}
              </button>
        ...
        <div className="mt-4 flex-1 flex flex-col">
          <h4 className="text-[#6a7257] uppercase font-bold text-[14px] mb-2 text-center">
            DETAILS FOR DOGBONES ASSEMBLY
          </h4>
          <textarea
            placeholder="Enter notes or parameters for Dogbones Assembly"
            value={metadata.notes || ''}
            onChange={e => handleChange('notes', e.target.value)}
            className="w-full flex-1 p-2 bg-black text-white text-[8px] border border-[#374151] rounded resize-none"
          />
        </div>
      </div>
    </div>
