...
export default function BodyPressureChartCard({ job }) {
  ...
  const [series, setSeries] = useState(() =>
    initialTransducers.map(([name, desc, value], i) => ({
      name,
      desc,
    ...
    const interval = setInterval(() => {
      setSeries(prev =>
        prev.map((tr, i) => ({
          ...
          data: [
            ...tr.data.slice(-29),
            getRandomPressure(tr.data[tr.data.length - 1] || 0)
          ]
        }))
      );
      setLabels(prev => [
  ...
  const options = {
    ...
    plugins: {
      ...
      tooltip: {
        ...
        callbacks: {
          label: context =>
            `${context.dataset.label}: ${context.parsed.y.toLocaleString()} PSI`
        }
      }
    },
    layout: { padding: 12 },
    scales: {
      x: {
        ticks: { color: '#b0b79f', maxTicksLimit: 8, font: { size: 12 } },
        grid: { color: '#23251f' }
      },
      y: {
        beginAtZero: true,
        ticks: { color: '#b0b79f', font: { size: 12 } },
        grid: { color: '#23251f' }
      }
    }
  };
