// ==============================
// services/assets.js — Postgres asset manager (Render + Local via ../db)
// ==============================
const db = require('../db');
...
async function getAllAssets() {
  const res = await db.query('SELECT * FROM assets ORDER BY id ASC');
  return res.rows;
}

async function getAssetsByStatus(status) {
  const res = await db.query('SELECT * FROM assets WHERE status = $1 ORDER BY id ASC', [status]);
  return res.rows;
}
...
async function deleteAsset(assetId) {
  const res = await db.query('DELETE FROM assets WHERE id = $1', [assetId]);
