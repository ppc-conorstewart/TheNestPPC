// ==============================
// server/routes/TorqueandServiceHub.js
// ==============================
...
const UPLOAD_FOLDER = path.join(__dirname, '..', 'uploads', 'torque-and-service-hub');
...
router.get('/', async (req, res) => {
    ...
    try {
        ...
        const docs = result.rows.map(row => ({
            ...row,
            file_url: `/api/torque-manuals/files/${row.id}`
        }));
        res.json(docs);
    } catch (err) {
...
router.post('/', upload.single('file'), async (req, res) => {
    try {
        ...
        await db.query(
            `INSERT INTO torque_and_service_hub_documents
...
router.patch('/:id', async (req, res) => {
    ...
    try {
        ...
        await db.query(
            `UPDATE torque_and_service_hub_documents SET file_name = $1, file_path = $2 WHERE id = $3`,
...
router.delete('/:id', async (req, res) => {
    ...
    try {
        await db.query(
            `UPDATE torque_and_service_hub_documents SET deleted = TRUE WHERE id = $1`,
            [id]
        );
        res.json({ success: true });
    } catch (err) {
