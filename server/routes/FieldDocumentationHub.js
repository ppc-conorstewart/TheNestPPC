// ==============================
// server/routes/FieldDocumentationHub.js
// ==============================
...
const UPLOAD_FOLDER = path.join(__dirname, '..', 'uploads', 'field-documentation-hub');
...
const storage = multer.diskStorage({
    ...
    filename: (req, file, cb) => {
        // Prevent duplicate file names per tab (appends timestamp)
...
router.get('/', async (req, res) => {
    ...
    try {
        ...
        const docs = result.rows.map(row => ({
            ...row,
            file_url: `/api/field-docs/files/${row.id}`
        }));
        res.json(docs);
    } catch (err) {
...
// ==============================
// Upload a document for a tab
// ==============================
router.post('/', upload.single('file'), async (req, res) => {
    try {
        const { tab } = req.body;
        if (!tab || !req.file) {
            return res.status(400).json({ error: 'Missing tab or file' });
        }
        const { originalname, filename, size, mimetype } = req.file;
        const file_path = filename;
        const file_name = originalname;
        await db.query(
            `INSERT INTO field_documentation_hub_documents
             (file_name, file_path, file_size, mime_type, tab)
             VALUES ($1, $2, $3, $4, $5)`,
            [file_name, file_path, size, mimetype, tab]
        );
...
router.patch('/:id', async (req, res) => {
    ...
    try {
        ...
        await db.query(
            `UPDATE field_documentation_hub_documents SET file_name = $1, file_path = $2 WHERE id = $3`,
...
router.delete('/:id', async (req, res) => {
    ...
    try {
        await db.query(
            `UPDATE field_documentation_hub_documents SET deleted = TRUE WHERE id = $1`,
            [id]
        );
        res.json({ success: true });
    } catch (err) {
