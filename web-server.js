...
// Resolve STATIC_DIR to an absolute path even if env gives a relative one
const STATIC_DIR_ENV = process.env.STATIC_DIR || path.join(__dirname, 'client', 'build');
const STATIC_DIR = path.isAbsolute(STATIC_DIR_ENV)
  ? STATIC_DIR_ENV
  : path.join(__dirname, STATIC_DIR_ENV);
